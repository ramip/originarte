...
class leobtslider extends Module
{
	...
	{
		/* Adds Module */
		if (parent::install() && $this->registerHook('displaySlideshow') && $this->registerHook('actionShopDataDuplication'))
		{
	...
	{
            if (!$this->isCached('leobtslider.tpl', $this->getCacheId()))
            {
		$this->_includeFile();
    ...
    $protocol = ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != "off") ? "https" : "http"). "://"; // add by rpc
		...
		$site_url = Tools::htmlentitiesutf8($protocol.$_SERVER['HTTP_HOST'].__PS_BASE_URI__).'cache/cachefs/'.$this->name;
	...
	{
            if(!$this->_prepareHook()) return;

                // Check if not a mobile theme
                if ($this->context->getMobileDevice() != false)
                    return false;
		$this->context->controller->addCSS( ($this->_path).'assets/styles.css', 'all');
		return $this->display(__FILE__, 'leobtslider.tpl', $this->getCacheId());
	}

        public function clearCache()
        {
                $this->_clearCache('leobtslider.tpl');
        }

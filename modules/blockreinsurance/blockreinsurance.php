...
class Blockreinsurance extends Module
{
	...
	public function hookFooter($params)
	{
		...
		if (!$this->isCached('blockreinsurance.tpl', $this->getCacheId()))
		{
			...
			$img_url = Tools::htmlentitiesutf8($protocol.$_SERVER['HTTP_HOST'].__PS_BASE_URI__).'modules/'.$this->name.'/img/'; // add by rpc 

			$infos = $this->getListContent($this->context->language->id);
			$this->context->smarty->assign(array('infos' => $infos, 'ruta'=>$img_url, 'nbblocks' => count($infos)));
		}
		return $this->display(__FILE__, 'blockreinsurance.tpl', $this->getCacheId());
	}

	public function hookDisplayExtraLeft($params){

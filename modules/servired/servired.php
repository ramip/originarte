...
Autor E-Mail: info@servycia.com
Fecha: Octubre 2015
Version : 0.7v4
Agradecimientos: Javier Barredo y Yago Ferrer por su m�dulo de pago  que se utiliz� como base de este m�dulo.
...
Notas para la versi�n de Servired 1.50 (26-10-2015)
--------------------------------

[-] Adaptaci�n a firma SHA256 obligatoria a partir del 22 de Noviembre de 2015 en Redsys
  - A�adida API apiRedsys.php
  - A�adido opci�n de pruebas en Redsys-d
  - Ocultada opci�n de tipo firma completa o ampliada
...
class servired extends PaymentModule
{
	...
	private function _displayForm(){
		...
		$entorno_d =  ($entorno==3) ? ' selected="selected" ' : '';
		...
		'<form action="'.$_SERVER['REQUEST_URI'].'" method="post">
			...
			<legend><img src="../img/admin/contact.gif" />'.$this->l('Configuraci&oacute;n del TPV').'</legend>
				<table border="0" width="680" cellpadding="0" cellspacing="0" id="form">
					...
					<tr><td width="215" style="height: 35px;">'.$this->l('Entorno de Servired').'</td><td><select name="urltpv"><option value="0"'.$entorno_real.'>'.$this->l('Real').'</option><option value="1"'.$entorno_t.'>'.$this->l('Pruebas en sis-t').'</option><option value="2"'.$entorno_i.'>'.$this->l('Pruebas en sis-i').'</option><option value="3"'.$entorno_d.'>'.$this->l('Pruebas en sis-d').'</option></select></td></tr>
					...
					<tr><td width="215" style="height: 35px;">'.$this->l('Clave secreta de encriptaci&oacute;n SHA256').'</td><td><input type="text" name="clave" value="'.Tools::getValue('clave', $this->clave).'" style="width: 200px;" /></td></tr>
					...
					<tr style="display:none"><td  width="215" style="height: 35px;">'.$this->l('Tipo de firma').'</td><td><input type="radio" name="tipofirma" id="tipofirma_c" value="1"'.$tipofirma_c.'/>'.$this->l('Completa').'<input type="radio" name="tipofirma" id="tipofirma_a" value="0"'.$tipofirma_a.'/>'.$this->l('Ampliada').'</td></tr>
	...
	$moneda = $moneda;
	$trans="0";
	//$url_merchant = Tools::getValue('notificacion', $this->notificacion);
	$url_merchant = 'http://'.$_SERVER['HTTP_HOST'].__PS_BASE_URI__.'modules/servired/respuesta_tpv.php';
	//$url_merchant = 'http://presta4.servycia.com/modules/servired/respuesta_tpv.php';
	//$urlOKKO="";
	...
	$miObj->setParameter("DS_MERCHANT_MERCHANTURL",$url_merchant);
...
//		$miObj->setParameter("Ds_Merchant_MerchantData",sha1($url_merchant));
//		$miObj->setParameter("Ds_Merchant_MerchantName",$this->nombre);
//		$miObj->setParameter("Ds_Merchant_PayMethods",$this->tipopago);
//		$miObj->setParameter("Ds_Merchant_Module","prestashop_2.8.2");

	//Datos de configuraci�n
	$version="HMAC_SHA256_V1";

	$kc = Tools::getValue('clave', $this->clave);
	...
	$signature = $miObj->createMerchantSignature($kc);
		...
		$smarty->assign(array(
			...
			'url_nueva' =>  Tools::getValue('urltpv', $this->urltpv),
			//'url_nueva' => 'http://sis-d.redsys.es/sis/realizarPago',
			'version' => $version,
			'params'=> $params,
			'signature'=> $signature
		));

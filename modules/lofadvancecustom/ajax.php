...
if( Tools::getIsset('lofajax') && Tools::getValue('task') == 'gethook'){
	$module_name = Tools::getValue('module_name');
	$hook_name = Tools::getValue('hook_name');
	$content = $module->getParamHooks( $module_name, $hook_name );
	die( $content );
}
if( Tools::getIsset('lofajax') && (Tools::getValue('task') == 'positionItem') && Tools::getValue('action') == 'dnd'){
...
if( Tools::getIsset('lofajax') && (Tools::getValue('task') == 'deleteItem')){
	$json_data = array();
	$json_data['result'] = 1;
...
if( Tools::getIsset('lofajax') && (Tools::getValue('task') == 'updateBlock')){
	$json_data = array();
	$json_data['result'] = 1;

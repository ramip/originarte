...
{/literal}
<div id="beezup_ho" class="col-sm-8">
    <p id="beezup_ho_info" class="info">{l s='Click here if you want to synchronize precise order' mod='beezup'} <i
                class="icon-plus-square"></i></p>
    <div id="beezup_ho_container">
        <form action="{$beezup_conf.BEEZUP_SITE_ADDRESS|escape:'htmlall':'UTF-8'}/modules/beezup/harvest_one.php"
            ...
            <div style="float:left;  font-weight: bold; font-size: 2em;">
                {l s='OR' mod='beezup'}
            </div>
            <div style="float:left">
                <div class="form-group">
                        ...
                        <div class="row">
                            <div class="margin-form col-sm-9 col-xs-12">
                                <input type="text" name="url" id="beezup_ho_url" value=""
                                       placeholder="{l s='Beezup order URL' mod='beezup'}" style="min-width: 300px"/>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    $(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data && data['id_order']) {
                        result = $("<p />").html($("<a />").attr("target", "_blank").attr("href", order_link + data['id_order']).text("{/literal}{l s='See synchronized order' mod='beezup'}{literal}" + " #" + data['id_order'])).addClass("alert alert-success");
                    } else if (data && data['error']) {
                        result = $("<p />").addClass("alert alert-danger").html(data['error'].split("\n").join("<br />"));
                    }
                    else {
                        result = $("<p />").addClass("alert alert-danger").text(JSON.stringify(data));
                    }
                    $("#beezup_ho_result").html(result).show();
                },
                error: function (data) {
                    var result = ""
                    result = $("<p />").addClass("alert alert-danger").text(data ? JSON.stringify(data) : {/literal}"{l s='Error' mod='beezup'}"{literal});
                    console.log(data);
                    $("#beezup_ho_result").html(result).show();
                }
            });
        ...
        $("#beezup_ho_info").click(function () {
            ...
            if (target.is(":visible")) {
                target.hide();
                $("#beezup_ho_info i").removeClass("icon-minus-square").addClass("icon-plus-square");
            }
            else {
                target.show();
                $("#beezup_ho_info i").removeClass("icon-plus-square").addClass("icon-minus-square");
            }
        });

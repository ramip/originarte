 ...
 * @copyright	Copyright (C) December 2010 leotheme.com <@emai:leotheme@gmail.com>.All rights reserved.
...
/**
 * leomanufacturerscroll Class
 */	
class leomanufacturerscroll extends Module
{
    ...
    */
	...
	{
		$this->name = 'leomanufacturerscroll';
		parent::__construct();			
		$this->tab = 'LeoTheme';			
		$this->version = '1.0.1';
		$this->author 	= 'LeoTheme';
		$this->displayName = $this->l('Leo Manufacturers Scroll');
		$this->description = $this->l('Leo Manufacturers Scroll - Support Responsive');	
		$this->Languages();   
	}
   /**
    * process installing 
    */
	function install(){
		global $cookie;
		$this->_clearCache('leomanufacturerscroll.tpl');
		if (!parent::install())
			return false;
		...
		if(!$this->registerHook('home'))
			return false;
		if(!$this->registerHook('actionObjectManufacturerDeleteAfter'))
			return false;
		if(!$this->registerHook('actionObjectManufacturerAddAfter'))
			return false;
		if(!$this->registerHook('actionObjectManufacturerUpdateAfter'))
			return false;
		...
		Configuration::updateValue($this->param_prefix.'_num_of_page', 5, true);
		...
		*/
		...
		Configuration::updateValue( $this->param_prefix.'_image_type', 'leo_manu' );
		return true;
	}
	...
	*/
	...
	function hookHeader($params) {
		...
		$this->context->controller->addJS(($this->_path).'assets/jquery.flexisel.js', 'all');
	}
	/*
	 * register hook right comlumn to display slide in right column
	 */
	...
	public function processHook( $params, $pos = 'home' ){
		...
		if (!$this->isCached('leomanufacturerscroll.tpl', $this->getCacheId())){
			$this->site_url = Tools::htmlentitiesutf8('http://'.$_SERVER['HTTP_HOST'].__PS_BASE_URI__);
			...
			$leoManufactureConfig = array("limit"=>$limit,"animate_time"=>$animate_time,"auto_play"=>$auto_play,"auto_time"=>$auto_time,"enable_responsive"=>$enable_responsive,"portraint_change_point"=>$portraint_change_point,"portraint_visible_items"=>$portraint_visible_items,"landscape_change_point"=>$landscape_change_point,"landscape_visible_items"=>$landscape_visible_items,"tablet_change_point"=>$tablet_change_point,"tablet_visible_items"=>$tablet_visible_items
			...
			$smarty->assign(
				array(
					...
					'leomanufacturers' => $manufacturers,
					'leoManufactureConfig' => $leoManufactureConfig
				)
			);
		}
		return $this->display(__FILE__, 'leomanufacturerscroll.tpl', $this->getCacheId());
	}

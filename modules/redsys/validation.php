	...
	if ($firma_local === $firma_remota
		...
		if ($accesoDesde === 'POST') {
			...
			if ($monedaOrig == $moneda && $totalOrig == $total && (int)$codigoOrig == (int)$codigo && $respuesta < 101 && checkAutCode($id_trans)) {
				...
				$redsys->validateOrder($pedido, _PS_OS_PAYMENT_, $totalCart, $redsys->displayName, null, $mailvars, (int)$cart->id_currency, false, $customer->secure_key);
				...
				if (!((int)$codigoOrig == (int)$codigo)) {
					escribirLog($idLog." -- "."El código de comercio no coincide.",$logActivo);
				}
				if (!checkAutCode($id_trans)){
					escribirLog($idLog." -- "."Ds_AuthorisationCode inválido.",$logActivo);
				}
				if ($error_pago=="no"){

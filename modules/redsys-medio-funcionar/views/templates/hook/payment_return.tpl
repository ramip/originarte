...
{if $status == 'ok'}
	<img src="{$this_path|escape:'htmlall'}modules/redsys/img/redsys.png" alt="Payment success" longdesc="Payment success" /><br /><br />
	<p>{l s='Your order on %s is complete.' sprintf=$shop_name mod='redsys'}
		<br /><br />
		{l s='Your check must include:' mod='redsys'}
		<br /><br />- {l s='Payment amount.' mod='redsys'} <span class="price"><strong>{$total_to_pay|escape:'htmlall'}</strong></span>

		{if !isset($reference)}
			<br /><br />- {l s='Do not forget to insert your order number #%d.' sprintf=$id_order mod='redsys'}
		{else}
			<br /><br />- {l s='Do not forget to insert your order reference %s.' sprintf=$reference mod='redsys'}
		{/if}
		<br /><br />{l s='An email has been sent to you with this information.' mod='redsys'}
		<br /><br /><strong>{l s='Your order will be sent as soon as we receive your payment.' mod='redsys'}</strong>

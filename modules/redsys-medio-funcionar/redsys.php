...
class Redsys extends PaymentModule
{
	...
	public function __construct()
	{
		...
		'REDSYS_MONEDA', 'REDSYS_TRANS', 'REDSYS_NOTIFICACION', 'REDSYS_SSL', 'REDSYS_ERROR_PAGO',
	...
	public function install()
	{
		...
		if (!parent::install()
			...
			|| !Configuration::updateValue('REDSYS_NOTIFICACION', 0)
	...
	public function uninstall()
	{
		...
		if (!Configuration::deleteByName('REDSYS_URLTPV')
			...
			|| !Configuration::deleteByName('REDSYS_NOTIFICACION')
	...
	private function _postProcess()
	{
		...
		if (Tools::isSubmit('btnSubmit'))
		{
			...
			Configuration::updateValue('REDSYS_NOTIFICACION', Tools::getValue('notificacion'));
		...
		$this->html .= '<div class="conf confirm"><img src="../img/admin/ok.gif" alt="'.$this->l('ok').'" /> '.$this->l('Configuraci&oacute;n actualizada').'</div>';
	}
	...
	public function hookPayment($params)
	{
		...
		{
			$ps_language = new Language((int)$this->cookie->id_lang);
		...
		$this->smarty->assign(array(
			...
			'titular' => ($this->cookie->logged ? $this->cookie->customer_firstname.' '.$this->cookie->customer_lastname : false),
			'nombre' => Tools::getValue('nombre', $this->nombre),
			'urltienda' => $urltienda,
			'notificacion' => Tools::getValue('notificacion', $this->notificacion),
			'productos' => $productos,
			'UrlOk' => 'https://'.$_SERVER['HTTP_HOST'].__PS_BASE_URI__.'index.php?controller=order-confirmation&id_cart='.(int)$id_cart.'&id_module='.(int)$this->id.'&id_order='.(int)$numpedido.'&key='.$customer->secure_key,
			'UrlKO' => $urltienda,
			'firma' => $firma,

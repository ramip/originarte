...
<div class="product-sellers">
	<h3 class="title_block"><a href="{$link->getPageLink('best-sales')|escape:'html'}">{l s='Top sellers' mod='blockbestsellers'}</a></h3>
		...
		<ul class="product_images">
			...
			<li class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} clearfix">
				<a href="{$product.link|escape:'html'}" title="{$product.legend|escape:'htmlall':'UTF-8'}" class="content_img clearfix">
					...
					<img src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'small_default')|escape:'html'}" alt="{$product.legend|escape:'htmlall':'UTF-8'}" />
				
				</a>
				<h4 class="s_title_block"><a href="{$product.link|escape:'html'}" title="{$product.legend|escape:'htmlall':'UTF-8'}">

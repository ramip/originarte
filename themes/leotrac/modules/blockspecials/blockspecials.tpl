...
{if $special}
		<ul class="products clearfix">
			<li class="product_image center_block">
				
				<a href="{$special.link|escape:'html'}"><img src="{$link->getImageLink($special.link_rewrite, $special.id_image, 'medium_default')}" alt="{$special.legend|escape:html:'UTF-8'}" title="{$special.name|escape:html:'UTF-8'}" /></a>
			
			<div class="desc_productleft">
				{if !$PS_CATALOG_MODE}
				...
				{if !$PS_CATALOG_MODE}
					...
					<span class="price price_container">{if !$priceDisplay}{displayWtPrice p=$special.price}{else}{displayWtPrice p=$special.price_tax_exc}{/if}</span>
				{/if}
				
			</div>
			</li>
		</ul>
		<p class="link_allview">

...
{if !$opc}<h1 class="title_block title_pading clearBoth">{l s='Addresses'}</h1>{else}<h2 class="clearBoth"><span>2</span> {l s='Addresses'}</h2>{/if}
...
{/if}
	<div class="addresses clearfix">
		...
		<div class="clearfix">
			<ul class="span6 address item" id="address_delivery" {if $cart->isVirtualCart()}style="display:none;"{/if}>
			</ul>
			<ul class="span6 address alternate_item {if $cart->isVirtualCart()}full_width{/if}" id="address_invoice">
			</ul>
		</div>

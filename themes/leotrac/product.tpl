...
<div id="primary_block" class="no-background row-fluid">
	...
	<!-- right infos-->
        ...
        {if isset($product) && $product->description}
            ...
            <div id="idTab1" class="rte">
                <h3 id="description-tab">{l s='Description'}</h3>
                {$product->description}
            </div>
        {/if}
	{/if}
							...
							<div class="content_prices">
								...
								<div class="price">
									...
									{if $priceDisplay >= 0 && $priceDisplay <= 2}
										...
										<p class="our_price_display1">{if $tax_enabled  && ((isset($display_tax_label) && $display_tax_label == 1) OR !isset($display_tax_label))}
    ...
    <!-- extraleft infos-->
		<div id="pb-extra-left-column" class="span3">
        	{$HOOK_EXTRALEFT}
          <!--  Cambiamos aqui las Plantillas -->
          <div id="plantillas">
            {if $attachments}<h4>{l s='Download'}</h4>{/if}
            {if isset($attachments) && $attachments}
		  		<ul id="idTab9" class="bullet">
		  		{foreach from=$attachments item=attachment}
		  			<li>
						<a href="{$link->getPageLink('attachment', true, NULL, "id_attachment={$attachment.id_attachment}")|escape:'html'}"><img src="{$img_dir}icon/arrow-download.png" alt="{l s='Download'}"/>{$attachment.name|escape:'htmlall':'UTF-8'}</a><br /><span>{$attachment.description|escape:'htmlall':'UTF-8'}</span></li>
		  		{/foreach}
				</ul>
			{/if}
           </div>
            <!--  Cambiamos aquí las Plantillas -->
        </div>
</div>

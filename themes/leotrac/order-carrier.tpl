...
{*if !$opc}
	<h1 class="title_block">{l s='Shipping:'}</h1>
{else}
	<h2><span>1</span> {l s='Delivery methods'}</h2>
{/if*}
	...
	{else}
		{if $recyclablePackAllowed}
			<div class=" control-group checkbox">
				...
				<div class="controls">
					<input class="input-xlarge" type="checkbox" name="recyclable" id="recyclable" value="1" {if $recyclable == 1}checked="checked"{/if} autocomplete="off"/>
				</div>
			</div>
		{/if}
...
*}                        
	...
	<div style="display: none;" id="extra_carrier"></div>
		...
		<p id="gift_div" class="textarea">
			<label for="gift_message">{l s='If you like, you can add a note to the gift:'}</label>
...
{*if $conditions AND $cms_id} { * bloque movido a order-payment * }
	<h3 class="condition_title">{l s='Terms of service'}</h3>
	<div class="checkbox">
		
		<input class="input-xlarge" type="checkbox" name="cgv" id="cgv" value="1" {if $checkedTOS}checked="checked"{/if} autocomplete="off"/>

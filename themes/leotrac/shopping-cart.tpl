...
<h1 id="cart_title" class="title_block" >{l s='Shopping-cart summary'}</h1>
			...
			<tr class="cart_total_price">
				...
				{if $voucherAllowed}
					...
					<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
						<fieldset>
							<p><label for="discount_name">{l s='Vouchers'}</label></p>
...
*}                
	...
	{/if}
		...
		{/if}
			...
			<tr class="cart_total_price">                            
				<td colspan="7" class="price total_price_container" id="total_price_container">
				{if $voucherAllowed}
					{if isset($errors_discount) && $errors_discount}
						<ul class="error">
						{foreach $errors_discount as $k=>$error}
...
<div class="order_delivery clearfix">
	...
	{else}
		...
		{foreach from=$formattedAddresses key=k item=address}
			<ul class="span6 address {if $address@last}last_item{elseif $address@first}first_item{/if} {if $address@index % 2}alternate_item{else}item{/if}">

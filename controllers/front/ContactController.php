...
class ContactControllerCore extends FrontController
{
	...
	public function postProcess()
	{
		...
		{
            $saveContactKey = $this->context->cookie->contactFormKey; // add by rpc
			$extension = array('.txt', '.rtf', '.doc', '.docx', '.pdf', '.zip', '.png', '.jpeg', '.gif', '.jpg');
			$fileAttachment = Tools::fileAttachment('fileUpload');
			$message = Tools::getValue('message'); // Html entities is not usefull, iscleanHtml check there is no bad html tags.
            $url = Tools::getValue('url'); // add by rpc
//			if (!($from = trim(Tools::getValue('from'))) || !Validate::isEmail($from)) // change by rpc
            if (!($from = trim(Tools::getValue('from'))) || !Validate::isEmail($from) || strpos($from, ".ru") !== FALSE)
            ...
            elseif ($url === false || !empty($url) || $saveContactKey != (Tools::getValue('contactKey'))) { // elseif add by rpc
	...
	public function initContent()
	{
        ...
        $contactKey = md5(uniqid(microtime(), true)); // add by rpc
        $this->context->cookie->__set('contactFormKey', $contactKey); // add by rpc

		$this->context->smarty->assign(array(
			'contacts' => Contact::getContacts($this->context->language->id),
			'message' => html_entity_decode(Tools::getValue('message')),
            'contactKey' => $contactKey, // add by rpc
		));

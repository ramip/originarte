...
class CmsControllerCore extends FrontController
{
	...
	public function init()
	{
		...
		if (isset($_POST['customcontact'])){
			if ( $_POST['bots'] != '' ) {
                header( 'Location: http://originarte.com/es/content/10-error-en-el-envio') ;
            } else {
				if ( $_POST['newsletter'] != '' ) {
				...
				$color = $_POST['color'];
				$otrocolor = $_POST['otrocolor'];
				$formato = $_POST['formato'];
				$formatocerrado = $_POST['formatocerrado'];
				$papel = $_POST['papel'];
				$papelportada = $_POST['papelportada'];
				...
				<p>Correo electrónico: $email </p>
					...
					<p>Color: $color </p> 
					<p>Otro color: $otrocolor </p> 
					<p>Formato abierto: $formato </p> 
					<p>Formato cerrado: $formatocerrado </p>
					<p>Papel: $papel </p>  
					<p>Papel portada: $papelportada </p> 
					<p>Acabado: $acabado </p> 
					<p>Mensaje: $mensaje </p> 
					<p>$newsletter</p>";
				...
				$mail->Subject = "Presupuesto personalizado";
				$mail->AltBody = 'Presupuesto';
				$mail->MsgHTML($formcontent);
				$mail->AddAddress('info@originarte.com', 'Originarte');
				$mail->AddAddress($email, 'Originarte');
				$mail->SMTPAuth = true;
				$mail->Username = 'info@originarte.com';
				$mail->Password = '77DYc0Ah';
				$exito = $mail->Send();
				if($exito){
					header( 'Location: http://originarte.com/es/content/8-presupuesto-personalizado-enviado') ;;
				}else{
